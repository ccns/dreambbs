...
* Xover 列表最後一頁為滿時的所顯示的訊息已改為與原版 MapleBBS 3.10 相同。\
  The message which appears when the last page of Xover list is full has been change to the same as the original MapleBBS 3.10.

* 新增在 Xover 列表中使用主題式瀏覽按鍵，而游標已在目標上時的訊息\
  Add a new message for indicating that the cursor is already on the target when using thread browsing hotkeys in Xover list 
...
* 修正了 BBS-Lua 與 BBS-Ruby 的問題。\
  ...
  * (v3.1.0-rc.1) BBS-Ruby 現已相容於 mruby 3.0.0\
    (v3.1.0-rc.1) BBS-Ruby is now compatible with mruby 3.0.0
...
+ 改進並擴充 Xover 列表系統\
  ...
  **The special values used in Xover list system are reassigned, please recompile.**
  ...
  - (v3.1.0-rc.1) `XO_RS + (RS_*)`: 主題式閱讀功能 Thread reading functionality
...
- 增加行數 Line insertions: 22850 (≈ 21.5%)
- 刪除行數 Line deletions: 19999 (≈ 18.8%)
