...
/* Originally derived from an Example of server using TCP protocol pp 284-5
...
main(int argc, char *argv[], char *envp[])
{
  ...
  err_init();			/* openlog() early - before we chroot() of
				   course */
  ...
  while (1)
  {
    ...
    /* Wait for a connection from a client process. This is an example of a
       concurrent server. */
    if (numready <= 0)
    {
...
/* Given a specific view, add the filename extension that we stripped off for
   the multiple views stuff */
...
HandleHTTPpost(CMDobj * cmd)
{
  ...
  if (CMDgetHTTPcmd(cmd) == POST)
  {
	...
	Fromhexstr(qp, qp);
      ...
      /* now add env vars from other stuff for http-cgi compliant child apps */
...
do_command(int sockfd)
{
  ...
  if (CMDgetCommand(cmd) != NULL)
  {
	...
	/* dgg patch for exec+: -- need to translate this
	   exec+:params:/go/path/prog<tab>! into calling program as this
  ...
  /* Set the environment variables for the scripts shell script writers
     everywhere.. */
  ...
  /* Ask items are shell scripts, not directories. Selector strings with '1/'
     should never be shell scripts */
...
GDfromUFS(char *pathname, int sockfd, boolean isGplus)
{
  ...
  /* open "." since we just moved there - makes it work when not chroot()ing
...
/* * This function lists out what is in a particular directory. * it also
   outputs the contents of link files. *

* It also checks for the existance of a .cache file if caching is * turned
   on... *
...
/* This processes a file containing any subset of Type, Name, Path, Port or
   Host, and returns pointers to the overriding data that it finds.

The caller may choose to initialise the pointers - so we don't touch them
   unless we find an over-ride. */
...
/* * This function opens the specified file, starts a zcat if needed, * and
   barfs the file across the socket. *
