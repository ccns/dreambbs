# 初步架站手冊 (for DreamBBS v2.0+)

這篇文件說明快速安裝本 BBS 系統的方法，適用於 v2.0 以後的版本。
...
本程式經測試可以安裝的環境：FreeBSD 11.1~12.0, GNU/Linux, Cygwin in Windows 7/10, Ubuntu on WSL on Windows 10
...
注意：帳號名稱不一定要是 `bbs`，這裡使用 `bbs` 僅為範例。
...
注意：群組名稱不一定要是 `bbs`，這裡使用 `bbs` 僅為範例。
...
== 以下請登入 bbs 帳號後操作 ==
    ...
    $ cd /home/bbs; git clone https://github.com/ccns/dreambbs; cd dreambbs; git checkout v2.0-artoria
...
接著產生設定檔：
若您的作業系統有安裝 `bmake` 套件 (在BSD-like系統中，改用 `make` 指令即可)，建議執行以下指令：

    $ bmake configure

若要改用 `cmake` 編譯，則建議使用以下指令：
...
(建議您自行檢視裡面的設定是否符合需求，以及視需要調整裡面的執行路徑)

在 WSL 上，要記得檢查 `cron` 是否已啟動：

    $ service cron status

至於設定 bbs 執行環境的部分：

在啟動 bbsd 主程式前，請務必先執行相關程式：
...
```
# 前略..
su bbs -c '/home/bbs/bin/bbsd -p 3456'  # 大於3000的備用port可這樣設定
