...
除法定義 | 取商 | 餘數範圍 | 取餘
--- | --- | --- | ---
floored (toward -∞) | `floor(x / (double)y)` <br> (x − y ∈ [−MIN − 1, MAX]) → `(x - (((x < 0) != (y < 0)) ? ((y < 0) ? -1 : 1) * (abs(y) - 1) : 0)) / y` | (y > 0) → [0, \|y\|) <br> (y < 0) → (−\|y\|, 0] | `x % y + (((x % y) && ((x < 0) != (y < 0))) ? y : 0)`
truncated <br> floored toward 0 | `trunc(x / (double)y)` <br> `x / y` | (x ≥ 0) → [0, \|y\|) <br> (x ≤ 0) → (−\|y\|, 0] | `x % y`
Euclidean | `copysign(1, y) * floor(x / fabs((double)y))` <br> (x − \|y\| ∈ [MIN − 1, MAX]) → `(x - ((x < 0) ? abs(y) - 1 : 0)) / y` | [0, \|y\|) | `x % y + ((x % y < 0) ? abs(y) : 0)`
ceiling (away from 0) | `copysign(ceil(fabs(x / (double)y)), x * copysign(1, y))` <br> (x ± \|y\| ∈ [MIN − 1, MAX + 1]) → `(x + (((x < 0)) ? -1 : 1) * (abs(y) - 1)) / y` | (x ≥ 0) → (−\|y\|, 0] <br> (x ≤ 0) → [0, \|y\|) | `x % y - ((x % y) ? (x < 0) ? -abs(y) : abs(y) : 0)`
ceiling (toward +∞) | `ceil(x / (double)y)` <br> (x + y ∈ [MIN − 1, MAX + 1]) → `(x + (((x < 0) == (y < 0)) ? ((y < 0) ? -1 : 1) * (abs(y) - 1) : 0)) / y` | (y > 0) → (−\|y\|, 0] <br> (y < 0) → [0, \|y\|) | `x % y - (((x % y) && ((x < 0) == (y < 0))) ? y : 0)`
rounded (.5 toward -∞) | `round(x / (double)y) - (fmod(x / (double)y, 1) == 0.5)` <br> (x + \|y\| / 2.0 ≤ MAX) → `(x + (((x < 0)) ? -1 : 1) * ((abs(y) - ((x < 0) == (y < 0))) >> 1)) / y` | (y > 0) → (−\|y\| / 2.0, \|y\| / 2.0] <br> (y < 0) → [−\|y\| / 2.0, \|y\| / 2.0) | `x % y - ((abs(x % y) > (((unsigned)abs(y) - ((x < 0) != (y < 0))) >> 1)) ? (x < 0) ? -abs(y) : abs(y) : 0)`
rounded (.5 toward 0) | `copysign(round(fabs(x / (double)y)) - (fmod(fabs(x / (double)y), 1) == 0.5), x * copysign(1, y))` <br> (x + \|y\| / 2.0 ≤ MAX + 1) → `(x + (((x < 0)) ? -1 : 1) * ((abs(y) - 1) >> 1)) / y` | (x ≥ 0) → (−\|y\| / 2.0, \|y\| / 2.0] <br> (x ≤ 0) → [−\|y\| / 2.0, \|y\| / 2.0) | `x % y - ((abs(x % y) > ((unsigned)abs(y) >> 1)) ? (x < 0) ? -abs(y) : abs(y) : 0)`
rounded (.5 to even) | `round(x / (double)y) - (fmod(x / (double)y, 2) == 0.5) + (fmod(x / (double)y, 2) == -0.5)` <br> (x + \|y\| / 2.0 ≤ MAX) → `(x + (((x < 0)) ? -1 : 1) * ((abs(y) - !((x / y) % 2U)) >> 1)) / y` | (q ≡ 0 (mod 2)) → [−\|y\| / 2.0, \|y\| / 2.0] <br> else → (−\|y\| / 2.0, \|y\| / 2.0) | `x % y - ((abs(x % y) > (((unsigned)abs(y) - !!((x / y) % 2U)) >> 1)) ? (x < 0) ? -abs(y) : abs(y) : 0)`
rounded (.5 away from 0) | `round(x / (double)y)` <br> (x + \|y\| / 2.0 ≤ MAX) → `(x + (((x < 0)) ? -1 : 1) * (abs(y) >> 1)) / y` | (x ≥ 0) → [−\|y\| / 2.0, \|y\| / 2.0) <br> (x ≤ 0) → (−\|y\| / 2.0, \|y\| / 2.0] | `x % y - ((abs(x % y) > (((unsigned)abs(y) - 1) >> 1)) ? (x < 0) ? -abs(y) : abs(y) : 0)`
rounded (.5 toward +∞) | `round(x / (double)y) + (fmod(x / (double)y, 1) == -0.5)` <br> (x + \|y\| / 2.0 ≤ MAX) → `(x + (((x < 0)) ? -1 : 1) * ((abs(y) - ((x < 0) != (y < 0))) >> 1)) / y` | (y > 0) → [−\|y\| / 2.0, \|y\| / 2.0) <br> (y < 0) → (−\|y\| / 2.0, \|y\| / 2.0] | `x % y - ((abs(x % y) > (((unsigned)abs(y) - ((x < 0) == (y < 0))) >> 1)) ? (x < 0) ? -abs(y) : abs(y) : 0)`
