...
- 新的程式碼不能將最新 ISO C++ 標準中的關鍵字當作變數／函式／型別名
    ...
    - 目前 (2020-02-24) 已基本相容 C++20 語法，可通過 `g++-8` 或 `clang++-6` 編譯並正常執行
...
- 不要手動最佳化運算式；現代許多編譯器已經能夠自動最佳化運算式（`gcc` 及 `clang` 在 `-O0` 下也會最佳化）
...
- 使用功能相似的 library/system 函式的考量重點
    ...
    - 優先度高到低：C standard library 函式 > glibc 函式 = POSIX 系統函式 > *NIX 系統函式 > 外部 library 函式
