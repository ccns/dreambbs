...
對應顯示座標之 x (PttBBS screen) | `01234567890789012345345678901` (個位) <br> `00000000001000111111111111122` (十位)
...
- 控制碼 `*[1m` 共 4 個字的原始字元座標為 `7`-`10`，對應的顯示座標之 `x` 則沒有定義，使用 PttBBS 的 screen 函數庫計算會得到與實際輸出位置不同的 `7`-`10`
- 控制碼 `*[m` 共 3 個字的原始字元座標為 `17`-`19`，對應的顯示座標之 `x` 則沒有定義，使用 PttBBS 的 screen 函數庫計算會得到與實際輸出位置不同的 `13`-`15`
