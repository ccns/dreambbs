...
int pip_basic_takeshower(void) /*¬~¾þ*/
{
	int lucky;
	d.shit -= 20;
...
int
pip_weapon_doing_menu(             /* ªZ¾¹ÁÊ¶Rµe­± */
int variance,
int type,
struct weapon *p)
{
...
int pip_see_doctor(void)	/*¬ÝÂå¥Í*/
{
	...
	{
		sprintf(buf, "§Aªº¯f­nªá %ld ¤¸³á....§A¤£°÷¿ú°Õ...", savemoney);
		vmsg(buf);
	}
...
#endif
#ifdef	HAVE_PIP_FIGHT
static int pip_fight_feed(void)     /* Áý­¹*/
{
	time_t now;
	char buf[256];
	int pipkey;
