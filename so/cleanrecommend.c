  ...
  int echo)
{
  ...
  if(vget(b_lines, 0, "¨Ï¥ÎªÌ:", cleanrecommend->userid, sizeof(cleanrecommend->userid), echo)
   && vget(b_lines, 0, "°Êµü:",cleanrecommend->verb, sizeof(cleanrecommend->verb), echo)
   && vget(b_lines, 0, "¯d¨¥:",cleanrecommend->msg, sizeof(cleanrecommend->msg), echo)
   && vget(b_lines, 0, "¤é´Á:",cleanrecommend->rtime, sizeof(cleanrecommend->rtime), echo))
    return 1;
  else
    return 0;
}
...
cleanrecommend_cleanall(
  XO *xo)
{
  if(vans("½T©w­n§R°£©Ò¦³ªº¯d¨¥¶Ü¡H[y/N]") == 'y')
  {
    unlink(xo->dir);
...
//          }
  ...
  for(i=0;i<rec_num(recommenddb,sizeof(RMSG));i++)
  {
    ...
    {
      sprintf(buf,"\033[m\033[1;33m   %12s¡G\033[36m%-54.54s \033[m%5.5s\n",rmsg.userid,rmsg.msg,rmsg.rtime);
    }
    f_cat(tmp,buf);
  }
