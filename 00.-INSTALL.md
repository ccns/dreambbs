...
另注意本專案有使用採用 BSD 授權條款的 piaip's more 閱讀程式 (在 `src/maple/pmore.c`)

若您同意該授權提及之相關使用方式，請自行將 `include/config.h` 中的相關定義：
```
#undef M3_USE_PMORE
```
修正為
...
#define M3_USE_PMORE
```
若您不需使用該套件，請自行至 `src/maple/Makefile` 依照相關提示修改設定，

或改以下列指令切換到 `Makefile` 中
沒有編譯 pmore 套件的 branch (目前為 **more.170810** )後，繼續安裝：
```
$ git checkout more.170810
```
