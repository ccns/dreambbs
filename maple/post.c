...
/*static inline void*/
	...
	void
move_post(	/* ±N hdr ±q currboard ·h¨ì board */
	HDR *hdr,
	char *board,
	int by_bm)
{
...
static int post_head(XO *xo);		/* Thor: ¦]¬° XoBM ­n¥Î */
	...
	XO *xo)
{
	...
	if (HAS_PERM(PERM_ADMIN))
	{
		...
		outs(ghdr->xname);
		outs("\033[m");      
        outs("\n \033[1;37m¡¹\033[m ¦nÅª³sµ²: " URL_PREFIX "/");
		outs(buf);
		outs("/");
		outs(ghdr->xname);
		outs("\033[m");
...
void
header_replace(		/* 0911105.cache: ­×§ï¤å³¹¼ÐÃD¶¶«K­×§ï¤º¤åªº¼ÐÃD */
  XO *xo,
  HDR *hdr)
{
  FILE *fpr, *fpw;
	...
	XO *xo)
{
	...
	static int
post_cross_terminator(	/* Thor.0521: ²×·¥¤å³¹¤jªk */
	XO *xo)
{
	...
	XO *xo)
{
    ...
    if(brd->battr & BRD_RSS)
	  prints("    RSS ¥\\¯à - " URL_PREFIX "/%s.xml ", brd->brdname);
    else
      prints("    RSS ¥\\¯à - Ãö³¬");
