...
{ 
  ...
  else
  {
    ...
    pmsg2("ªO¥D²§°Ê¤w¥[¤J¯¸ªø¤é»x");
    if (!vget(b_lines, 0, "½Ð¿é¤J²§°Ê²z¥Ñ¡G", why,40, DOECHO))
    {   
      sprintf(why, "¥¼¿é¤J²z¥Ñ¡A¸T¤î²§°Ê");
      pmsg2("½Ð¿é¤J²§°Ê²z¥Ñ");
      return;
    } 
    sprintf(tmp, "\n\n%s %-12s ¹ï¨Ï¥ÎªÌ %-12s °õ¦æªO¥D²§°Ê\n²z¥Ñ: ", Now(), cuser.userid, u->userid);
  ...
  int adm)
{
  ...
  if(!adm)
  {
    ...
    if(select != 6)
    {
      days = vans("³B»@´Á­­ 1)¤@¬P´Á 2)¨â¬P´Á 3)¤T¬P´Á 4)¤@­Ó¤ë 5)¥Ã¤[ [1] ") - '0';
  ...
  int cross)
{
  ...
  if((adm & DENY_DAYS) && !(adm & DENY_MODE_GUEST))
  {
    ...
    else if(adm & DENY_DAYS_3) { cdays = "¤T¬P´Á";x.deny += 86400 * 21;}
  ...
  int adm)
{
  ...
  if (adm)
  {
    if(supervisor)
    {
      ...
      {
        char tmp[80], why[80];
        pmsg2("¬d¸ß°Ê§@¤w¥[¤J¯¸ªø¤é»x");
        if (!vget(b_lines, 0, "½Ð¿é¤J²z¥Ñ¡G", why, 40,DOECHO))
        { 
          sprintf(why, "¥¼¿é¤J²z¥Ñ¡A¸T¤î¬d¸ß");
          pmsg2("½Ð¿é¤J¬d¸ß²z¥Ñ");
          return;
        } 
      ...
      adm = vans("³]©w 1)¸ê®Æ 2)Åv­­ 3)³s§¤³B»@ 4)³æ¤H³B»@ 5)°ò¥»Åv­­ 6)ºX¼Ð 7)SU Q)¨ú®ø [Q] ");      
    }
    else
    {
      if (!str_cmp(cuser.userid, ELDER))
      ...
      else
      {
        char tmp[80], why[80];
        pmsg2("¬d¸ß°Ê§@¤w¥[¤J¯¸ªø¤é»x");
        if (!vget(b_lines, 0, "½Ð¿é¤J¬d¸ß²z¥Ñ¡G", why,40,DOECHO))
        { 
          sprintf(why, "¥¼¿é¤J²z¥Ñ¡A¸T¤î¬d¸ß");
          pmsg2("½Ð¿é¤J¬d¸ß²z¥Ñ");
          return;
...
/*    vget(i, 0, "½Ð½T»{±K½X¡G", buf, PASSLEN, NOECHO); */
  ...
  if (adm)
  {
    ...
    vget(++i, 0, "¹Ú¹ô¡G", buf, 10, GCARRY);
    if ((num = atoi(buf)) >= 0)
      x.money = num;

    sprintf(buf, "%d", u->point1);
    vget(++i, 0, "Àu¨}¿n¤À¡G", buf, 10, GCARRY);
    if ((num = atoi(buf)) >= 0)
      x.point1 = num;

    sprintf(buf, "%d", u->point2);
    vget(++i, 0, "¦H¤å¡G", buf, 10, GCARRY);
    if ((num = atoi(buf)) >= 0)
      x.point2 = num;
  ...
  char IdleState[][IDLEN]=
{
  ...
  vget(b_lines - 1, 0, "²z¥Ñ:[0]µo§b (1)±µ¹q¸Ü (2)³V­¹ (3)¥´½OºÎ (4)¸Ë¦º (5)­ú­ú (6)¨ä¥L (Q)¨S¨Æ:",
      buf, 2, DOECHO);
  ...
  BRD *brd)
{
  ...
  {
    char tmp[80], why[80];
    pmsg2("­×§ï°Ê§@¤w¥[¤J¯¸ªø¤é»x");
    if (!vget(b_lines, 0, "½Ð¿é¤J­×§ï²z¥Ñ¡G", why,40, DOECHO))
    { 
      sprintf(why, "¥¼¿é¤J²z¥Ñ¡A¸T¤î­×§ï");
      pmsg2("½Ð¿é¤J­×§ï²z¥Ñ");
      return 0;
    } 
    sprintf(tmp, "\n\n%s %-12s ¹ï¬ÝªO %-12s °õ¦æ­×§ï°Ê§@\n²z¥Ñ: ", Now(), cuser.userid, brd->brdname);
    f_cat(FN_BLACKSU_LOG, tmp);
    f_cat(FN_BLACKSU_LOG, why);
  }
  ...
  /* cache.091124: §ï¥Î¸û«K§Qªº¬ÝªOÅv­­³]©w */
  switch (vget(++i, 0, "¬ÝªOÅv­­ A)¤@¯ë B)¦Û©w C)¯µ±K D)¦n¤Í¡H[Q] ", buf, 3, LCECHO))
  {
  case 'c':
    brd->readlevel = (PERM_SYSOP | PERM_BOARD);	/* ¯µ±K¬ÝªO */
    brd->postlevel = PERM_POST;
    brd->battr |= (BRD_NOSTAT | BRD_NOVOTE);
    break;

  case 'd':
    brd->readlevel = PERM_SYSOP;	/* ¦n¤Í¬ÝªO */
    brd->postlevel = PERM_POST;
    brd->battr |= (BRD_NOSTAT | BRD_NOVOTE);
    break;
#else
  switch (vget(++i, 0, "¬ÝªOÅv­­ A)¤@¯ë B)¦Û©w¡H[Q] ", buf, 3, LCECHO))
  {
#endif
  ...
  /* cache.090928 ¬ÝªO¤¬¥¸ÄÝ©Ê - ¦³±À¤å©M¦Û©w±À¼N¤å(¹w³]) */
  if((brd->battr & BRD_PUSHSNEER) && (brd->battr & BRD_PUSHDEFINE))
    brd->battr &= ~BRD_PUSHSNEER;

  /* cache.090928 ¬ÝªO¤¬¥¸ÄÝ©Ê - ·í¬ÝªO°ßÅª®É¤]¸T¤î±À¤å */
  if(brd->battr & BRD_NOREPLY)
    brd->battr |= BRD_PRH;

  /* cache.090928 ¬ÝªO¤¬¥¸ÄÝ©Ê - ¤£¥i±À¤å®É²M°£©Ò¦³±À¤å¼Ò¦¡©M­­¨î */
...
int
a_editbrd(void)		/* cache.100618: ­×§ï¬ÝªO¿ï¶µ */
{
  int bno;
  BRD *brd;
  char bname[IDLEN + 1];
...
u_verify(void)
{
  ...
  if (!dashf(keyfile))
  {
    zmsg("¨S¦³±zªº»{ÃÒ½X³á¡A½Ð­«¶ñ email address");
    return XEASY;
  }
  ...
  if (vget(b_lines, 0, "½Ð¿é¤J»{ÃÒ½X¡G", inbuf, 8, DOECHO))
  {
    if (strncmp(key, inbuf,7))
    {
      zmsg("©êºp¡A±zªº»{ÃÒ½X¿ù»~¡C");
    ...
    else
    {
      ...
      sprintf(buf, "key»{ÃÒ:%s", cuser.email);
